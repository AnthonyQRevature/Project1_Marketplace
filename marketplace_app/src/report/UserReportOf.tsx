...
function UserReports() {
  ...
  const [] = useLoader<UserProfile>(makeEndpoint(Number(user_id)));
  ...
  return (
    <div className="report-container">
      ...
      {activeTab === "view" && <ViewUserReports user_id={user_id} auth={auth} setMessage={setMessage} />}
    </div>
  );
}
...
{
  return {endpoint: `/users/${id}/reports/of`, method: "GET"};
}
...
function ViewUserReports({ 
  ...
  const fetchReports = async () => {
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE}/users/${user_id}/reports/of`, {
        method: "GET",
        headers: {
